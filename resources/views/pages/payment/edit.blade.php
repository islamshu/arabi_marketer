    ...
    <div class="row">
        ...
        <div class="form-group col-md-6">
            ...
            <input type="text" name="title_ar" required class="form-control"
                value="{{ $payment->getTranslation('title', 'ar') }}" id="title_ar">
        </div>
        <div class="form-group col-md-6">
            <label for="email"> العنوان بالانجليزية : <span class="required"></span></label>
            <input type="text" name="title_en" required class="form-control"
                value="{{ $payment->getTranslation('title', 'en') }}" id="title_en">
        </div>
...
<script>
    ...
    $('#edit_form_new').on('submit', function(e) {
        ...
        updatefile("{{ route('update_payments', $payment->id) }}", 'post', formData, 'Edit successfully') ;
